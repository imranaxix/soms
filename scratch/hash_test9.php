...
$payload = [
    ...
    'pp_CNIC'               => '345678',
    'pp_ReturnURL'          => 'http://contempt-racism-paralyses.ngrok-free.dev/jazzcash/callback',
];
...
foreach ($filtered as $key => $value) {
    // JazzCash has a known bug where they strip hyphens, or limit URL length to 50 chars maybe?
    // Let's try hashing without hyphens
    // Or maybe limit length?
    if ($key === 'pp_ReturnURL') {
        $valToHash = str_replace('-', '', $value);
    } else {
        $valToHash = $value;
    }
    $hashString .= '&' . $valToHash;
}
...
echo "No hyphens in hash: Code: " . $resp->json('pp_ResponseCode') . " | Msg: " . $resp->json('pp_ResponseMessage') . "\n";
