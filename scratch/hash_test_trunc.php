...
foreach ($filtered as $key => $value) {
    if ($key === 'pp_ReturnURL') {
        // JazzCash portal might truncate to 50 chars when saving:
        // https://contempt-racism-paralyses.ngrok-free.dev (50 chars exactly)
        $hashString .= '&' . substr($value, 0, 50);
    } else {
        $hashString .= '&' . $value;
    }
}
...
echo "Truncated 50 Hash test: " . $resp->json('pp_ResponseCode') . " | Msg: " . $resp->json('pp_ResponseMessage') . "\n";
