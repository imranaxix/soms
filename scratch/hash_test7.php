...
function testHash(array $payload, string $salt, bool $excludePassword) {
    $filtered = array_filter($payload, function ($value, $key) use ($excludePassword) {
        if ($key === 'pp_SecureHash') return false;
        if ($excludePassword && $key === 'pp_Password') return false;
    ...
    echo "Exclude Password: " . ($excludePassword ? "YES" : "NO") . "\n";
    echo "Code: " . $resp->json('pp_ResponseCode') . " | Msg: " . $resp->json('pp_ResponseMessage') . "\n";
    echo "Jazzcash returned Hash: " . $resp->json('pp_SecureHash') . "\n";
    echo "Our Hash:               " . $payload['pp_SecureHash'] . "\n\n";
}

testHash($payload, $salt, true);
testHash($payload, $salt, false);
