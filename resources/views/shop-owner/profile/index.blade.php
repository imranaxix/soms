...
<main class="p-6 max-w-4xl mx-auto space-y-6">
    ...
    <div class="bg-white rounded-3xl border border-neutral-200 overflow-hidden shadow-sm">
        ...
        <div class="p-8">
            <form action="{{ route('shop.profile.update') }}" method="POST" class="space-y-8">
                @csrf
                @method('PUT')
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                    <div class="space-y-2">
                        <label class="text-[11px] font-bold text-neutral-400 uppercase tracking-widest ml-1">Business Name</label>
                        <input type="text" name="business_name" class="w-full px-5 py-3.5 bg-neutral-50 border border-neutral-200 rounded-xl text-sm font-bold text-neutral-900 focus:ring-4 focus:ring-primary-500/10 focus:border-primary-500 outline-none transition-all placeholder:text-neutral-300" value="{{ old('business_name', $user->business_name) }}">
                ...
                <div class="pt-8 border-t border-dashed border-neutral-200 flex justify-end gap-4">
                    <a href="{{ route('shop.profile') }}" class="px-8 py-3.5 text-neutral-500 font-bold rounded-xl hover:bg-neutral-50 transition-colors text-sm">Discard Changes</a>
                    <button type="submit" class="px-10 py-3.5 bg-primary-600 text-white font-black rounded-xl hover:bg-primary-700 shadow-xl shadow-primary-500/20 transition-all text-sm transform hover:-translate-y-0.5">Save Profile</button>
                </div>
            </form>
        </div>
    </div>
